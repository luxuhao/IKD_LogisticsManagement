...
public class ImageUtils
{
    ...
    public void loadLocalImage(String imageUrl, ImageView imageView){
    ...
    public void loadImage(String imageUrl, ImageView imageView){
        if(imageUrl.isEmpty()){return;}
        String loadImageUrl = "";
        if(imageUrl.length() > "http".length() &&
                imageUrl.toLowerCase().substring(0,"http".length()).equals("http")){
            loadImageUrl = imageUrl;
        }else{
            loadImageUrl = webHost + imageUrl;
        }
        Glide.with(mContext).load(loadImageUrl).into(imageView);
    }
